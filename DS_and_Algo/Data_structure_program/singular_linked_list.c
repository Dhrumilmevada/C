/************************************************
*
* Wrire a program to create a singilar linked list and perform verious instruction like insertion and deletion of node. 
*
*
******************************************************/
...
/*****************************************************************************************
*
*	@file	singular_linked_list.c
*	@author	Dhrumil Mevada
*	@brief	This file will create singular linked list and perform verious opertaion on it.
...
int main()
{
    ...
    while(1)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            delete_node();		//to delete a node to lost
            break;
        case 3:
...
void insert_node_first(){
    ...
    if(head==NULL){
        head = last = node;			//insert a node to list if list is empty
        node_cnt++;
    }
    else{
        node->next = head;
        head = node;				//insert a node to list at start of list
        node_cnt++;
    }
}
...
void insert_node_last(){
    ...
    if(head==NULL){
        ...
        last->next= node;
        last = node;				//insert a node to list at last of list
        node_cnt++;
    }
}
...
void insert_node_between()
{
	...
	if(node_cnt==0){
            ...
            scanf("%d",&position);					//if list is empty then execute this one 
            if(position>=1 && position<=node_cnt+1)
            break;
        }
        else{
            printf("\nEnter a number between(1-%d):- ",node_cnt);
            scanf("%d",&position);					//if list has a list of node then perform this
...
void delete_node(){
    ...
    else
    {
        ...
        if(position>=1 && position<=node_cnt)		//to check position is correctly entered or not
        {
            ...
            {
                head = head->next;
                temp->next = NULL;			//if postion is 1 then delete node at start of list
                free(temp);
                node_cnt--;
            }
            else if(position<=node_cnt){
                ...
                temp  = head;
                for(i=1; i<position-1; i++)
                {					//else delete node from specified location
                    temp= temp->next;			//travesal to specified location
                }
                target = temp->next;
...
void print_linked_list(){
    ...
    {
        printf("%d->",temp->val);			//travesal to list and print value of each node.
    }
    printf("\n%d",node_cnt);
}
